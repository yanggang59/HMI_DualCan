...
int INIParser::ReadINI(string path)
{
	...
	while (getline(in_conf_file, str_line))
	{
		...
		{
			ININode ini_node(str_root, str_key, atof(str_value.c_str()));
...
//************************************************************************
float INIParser::GetValue(string root, string key)
{
	map<string, SubNode>::iterator itr = map_ini.find(root);
	map<string, float>::iterator sub_itr = itr->second.sub_node.find(key);
...
int INIParser::WriteINI(string path)
{
	...
	for (map<string, SubNode>::iterator itr = map_ini.begin(); itr != map_ini.end(); ++itr)
	{
		...
		for (map<string, float>::iterator sub_itr = itr->second.sub_node.begin(); sub_itr != itr->second.sub_node.end(); ++sub_itr)
		{
...
//************************************************************************
vector<ININode>::size_type INIParser::SetValue(string root, string key, float value)
{
...
void INIParser::Travel()
{
	for (map<string, SubNode>::iterator itr = this->map_ini.begin(); itr != this->map_ini.end(); ++itr)
	{
		//root
		cout << "[" << itr->first << "]" << endl;
		for (map<string, float>::iterator itr1 = itr->second.sub_node.begin(); itr1 != itr->second.sub_node.end();
			++itr1)
		{
